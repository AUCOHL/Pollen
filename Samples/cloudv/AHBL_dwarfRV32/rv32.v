...
`define     ADDR_SELNONE    32'h00440044
...
`ifdef _DBUG_
	always @ (*)
		$display ("bsel = %b", bsel);
	always @ (*)
		$display ("do_before = %h, do_ext = %h", di, do);
...
always @ (posedge clk or posedge rst)
	...
	else if(~cyc & (cu_alu_inst_1 | cu_br_inst_1)) begin
		CF <= cf;
		ZF <= zf;
		VF <= vf;
		SF <= sf;
...
`ifdef _DBUG_
	always @(*)
		if (compressed)
			$display ("decompressed_inst = %h", d16ic_32);
`endif
`endif
...
always @ (posedge clk or posedge rst)
	if(rst)
	   	PC1 <= 32'b0;
	else if(cyc & ~hold) begin
            PC1 <= PC;
`ifdef _TRACE_
		if(!(~cu_pc_reliable | (intf & ~cu_int_ecall)))
			$display("0x%h", PC);
`endif
	end
            ...
            case(fun3)
                3'b000:begin             //C.ADDI4SPN
                   //addi rd0, x2, nzuimm[9:2].
					Instout = {2'b00,Inst[10:7],Inst[12:11],Inst[5],Inst[6],2'b00,5'b00010,3'b000,2'b01,Inst[4:2],7'b0010001};
                end
                ...
                3'b011:begin            //C.LUI,C.ADDI16SP
                    case(Brs1)
						5'b00010: begin     //C.ADDI16SP
							...
							Instout = {{3{Inst[12]}},Inst[12],Inst[4:3],Inst[5],Inst[2],Inst[6],4'd0,Brs1,3'b000,Brs1,7'b0010001};
						end
                    default: begin      //C.LUI
                    //lui rd, nzuimm[17:12].
